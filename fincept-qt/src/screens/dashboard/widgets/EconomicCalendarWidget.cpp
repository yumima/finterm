...
void EconomicCalendarWidget::apply_styles() {
    ...
    status_label_->setStyleSheet(QString("color: %1; padding: 16px; background: transparent;")
                                     .arg(ui::colors::TEXT_SECONDARY()));
}
...
void EconomicCalendarWidget::populate(const QJsonArray& events) {
    ...
    for (const auto& v : events) {
        ...
        cty_lbl->setStyleSheet(QString("color: %1; background: transparent;").arg(ui::colors::CYAN()));
        ...
        act_lbl->setStyleSheet(QString("color: %1; font-weight: bold; background: transparent;")
        ...
        imp_lbl->setStyleSheet(
            QString("color: %1; font-weight: bold; background: transparent;").arg(imp_color));
