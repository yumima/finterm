...
void PortfolioSectorPanel::update_donut() {
    ...
    for (int i = 0; i < sorted.size(); ++i) {
        ...
        name->setStyleSheet(active ? QString("color:%1; font-size:12px; font-weight:700;").arg(ui::colors::AMBER())
                                   : QString("color:%1; font-size:12px;").arg(ui::colors::TEXT_PRIMARY()));
        row->addWidget(name);
        row->addStretch();
        ...
        pnl_lbl->setStyleSheet(QString("color:%1; font-size:12px;").arg(pc));
        row->addWidget(pnl_lbl);

        auto* weight = new QLabel(QString(" %1%").arg(QString::number(sorted[i].second, 'f', 1)));
        weight->setStyleSheet(QString("color:%1; font-size:12px; font-weight:600;").arg(ui::colors::TEXT_SECONDARY()));
...
void PortfolioSectorPanel::update_correlation() {
    ...
    if (holdings_.size() < 2) {
        ...
        msg->setStyleSheet(QString("color:%1; font-size:12px;").arg(ui::colors::TEXT_SECONDARY()));
    ...
    for (int r = 0; r < n; ++r) {
        ...
        row_label->setStyleSheet(QString("color:%1; font-size:12px; font-weight:700;").arg(ui::colors::TEXT_SECONDARY()));
