...
ImportPortfolioDialog::ImportPortfolioDialog(const QVector<portfolio::Portfolio>& portfolios, QWidget* parent)
    ...
    setStyleSheet(QString("QDialog { background:%1; color:%2; }"
                          ...
                          "  padding:6px 10px; font-size:12px; }"
                          "QLineEdit:focus { border-color:%6; }"
                          "QRadioButton { color:%2; font-size:12px; spacing:6px; }"
    ...
    // (No "download demo JSON" template: finterm ships no fabricated sample
    // holdings/prices. Import a real exported portfolio via BROWSE.)
