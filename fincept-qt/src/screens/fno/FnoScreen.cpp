...
constexpr TabDef kTabDefs[FnoScreen::TabCount] = {
    {"Chain", "Live option chain"},
    {"Builder", "Strategy builder + payoff"},
    {"OI", "Open Interest analytics"},
    {"Multi-Stra", "Multi straddle / strangle charts"},
    {"Screener", "Chain screener"},
};
...
void FnoScreen::setup_ui() {
    ...
    // Pre-create all slots with placeholders so QStackedWidget indices map
...
void FnoScreen::ensure_tab_built(SubTab which) {
    ...
    // All five F&O sub-tabs are real (no more placeholders).
}
