...
bool TtsService::is_available() const {
    ...
    if (provider.isEmpty() || provider == QStringLiteral("none"))
        return false;
    if (provider != QStringLiteral("piper"))
        return false;
    const QString model = get_setting("tts.model_path");
...
void TtsService::speak(const QString& text) {
    ...
    env.insert("FINCEPT_TTS_MODEL", get_setting("tts.model_path"));
    ...
    LOG_INFO(kTtsTag, QString("speaking via Piper (model=%1, %2 chars)")
                          .arg(get_setting("tts.model_path"), QString::number(text.size())));
    proc_->start();
