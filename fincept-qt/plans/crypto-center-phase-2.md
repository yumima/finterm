...
| D2 | Pool routing | **`pool: "auto"`** on every request. | $FNCPT is post-graduation (bonding curve complete), so PumpPortal will pick PumpSwap. If finterm ever launches a new token pre-graduation, the same code routes it correctly. |
